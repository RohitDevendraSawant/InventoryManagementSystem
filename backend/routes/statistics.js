...
router.post("/getCount",fetchAssistant,async(req,res)=>{
    try {
        ...
        if (lab  && category  && specification) {
            console.log("1");
            const count = await Item.find({$and : [{lab : lab},{$and : [{category : category},{specification : specification}]}]}).count();
            return res.status(200).json(count);
        }
        else if (lab && category) {
            console.log("2");
            const count = await Item.find({$and : [{lab : lab},{category : category}]}).count();
            return res.status(200).json(count);
        }
        else if (category && specification) {
            console.log("3");
            const count = await Item.find({$and : [{category : category},{specification : specification}]}).count();
            return res.status(200).json(count);
        }
        else if (lab && specification) {
            console.log("4");
            const count = await Item.find({$and : [{lab : lab},{specification : specification}]}).count();
            return res.status(200).json(count);
        }
        else if (lab) {
            console.log("5");
